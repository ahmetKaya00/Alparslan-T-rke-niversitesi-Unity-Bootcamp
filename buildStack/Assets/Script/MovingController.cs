...
{
    public static MovingController CurrentCube { get; private set; }
    public static MovingController LastCube { get; private set; }
    ...
    {
        if (LastCube == null)
            LastCube = GameObject.Find("Start").GetComponent<MovingController>();
        CurrentCube = this;
    ...
    private void SplitCubeOnZ(float breakZ, float direction)
    {
        ...
        float newPosition = LastCube.transform.position.z + (breakZ / 2);
        ...
        float fallingBlockZPos = cubeEgde + fallingBlockSize / 2f;
    ...
    }private void SplitCubeOnX(float breakZ, float direction)
    {
        ...
        float newPosition = LastCube.transform.position.x + (breakZ / 2);
        transform.localScale = new Vector3(newXSize,transform.localScale.y, transform.localScale.z);
        transform.position = new Vector3(newPosition,transform.position.y, transform.position.z );

        float cubeEgde = transform.position.x + (newXSize / 2 * direction);
        float fallingBlockXPos = cubeEgde + fallingBlockSize / 2f;
